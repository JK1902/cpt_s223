...
{
    std::cout << "--------------------------------------------------------------------------------" << std::endl
              << "The objective of the game is to match Linux cmds to appropriate descriptions of those cmd. 
              If a command is matched, then you earn the points for that cmd. The if the command is not matched, then you lose that many points. Yes, negative point totals are possible. Before the game, you select the number of match questions to be asked.
              The game continues until the number is reached.(Press Enter to return to menu)" << std::endl;
...
void addCommand()
{
    ...
    // Get information on the new cmd
    std::cout << "--------------------------------------------------------------------------" << std::endl
              << "What is the name of this cmd?" << std::endl;
    ...
    // Append the new cmd to the list
...
void remCommand()
{
    ...
    // Get the name of the target cmd
    std::cout << "--------------------------------------------------------------------------" << std::endl
              << "What cmd do you wish to delete?" << std::endl;
    
    std::cin.ignore(INT64_MAX, '\n'); // Eats previous enter press
    std::getline(std::cin, name);

    // Search for the index of the cmd
    for (int i = 0; i < listSize; i++)
    ...
    // Alert the user if the cmd was not found
    std::cout << "This cmd was not found! Did you spell it correctly?" << std::endl;
}

// Display all cmds in the list
void dispCommands()
{
